...
- `LIFETIME-004` `BoundaryTooShort`: *Error*, from `sema/outlives_check.rs` (Phase E2 + E4, `docs/OUTLIVES_RULES.md`). "value doesn't live long enough for declared lifetime `L`". Fires at either boundary shape §3 names — a call to a function with exactly one `[lifetime L]` param, or a struct literal for a struct with exactly one `[lifetime L]` (`edge` or not) — where the actual argument/field value at a `&L T` position resolves to a local bound to a loan (`let p = &x`), and that loan's own computed region (Phase E1) doesn't reach the site's point — the loan was already killed (e.g. by a reassignment of the borrowed-from place) before it got here. Secondary span points at the loan itself. Suggestion: move the call/construction before the loan's last use, or restructure so the loan lives at least as long as this site.
- `LIFETIME-006` `NonLocalBoundaryValue`: *Error*, same pass. "can't verify this value satisfies declared lifetime `L`". The conservative-reject case: the value resolves to a local, but that local is neither one of the caller's own parameters nor bound to any known loan (typically because it came from another call's return value — a call result is never itself registered as a loan). v1 can't prove it's fine, so it doesn't allow it, even in cases that are actually safe. Suggestion: bind it to a local first so its lifetime can be traced, or pass/store a fresh borrow directly. `LIFETIME-005` is reserved for Phase E3 (multi-lifetime `outlives` propagation), not yet built — see `docs/OUTLIVES_RULES.md` §9.
