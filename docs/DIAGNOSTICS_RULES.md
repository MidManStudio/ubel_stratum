...
| LIFETIME-004 | BoundaryTooShort |
| LIFETIME-005 | OutlivesConstraintViolated |
| LIFETIME-006 | NonLocalBoundaryValue |
...
- `LIFETIME-004` `BoundaryTooShort`: *Error*, from `sema/outlives_check.rs` (Phase E2 + E4, `docs/OUTLIVES_RULES.md`). "value doesn't live long enough for declared lifetime `L`". Fires at either boundary shape §3 names — a call to a function with a `[lifetime L]` param, or a struct literal for a struct with `[lifetime L]` (`edge` or not) — where the actual argument/field value at a `&L T` position resolves to a local bound to a loan (`let p = &x`), and none of that loan's own computed regions (Phase E1) reach the site's point — the loan was already killed (e.g. by a reassignment of the borrowed-from place) before it got here. Secondary span points at the loan itself. Suggestion: move the call/construction before the loan's last use, or restructure so the loan lives at least as long as this site.
- `LIFETIME-005` `OutlivesConstraintViolated`: *Error*, same pass, Phase E3. "`L` doesn't actually outlive `M` at this site". Fires when a boundary site declares two or more lifetimes with a `where L outlives M` constraint between them, both sides independently pass their own `LIFETIME-004`/`LIFETIME-006` check, but the *relative* ordering the signature promises doesn't hold — `M`'s resolved region (a fresh borrow's singleton point, or a live loan's own region) isn't a subset of `L`'s. One asymmetric special case: if `L`'s side resolves to one of the *caller's own* forwarded parameters, the constraint is accepted unconditionally regardless of what `M` resolves to (a forwarded parameter is valid for the caller's entire body, necessarily a superset of anything else traceable within it) — but the reverse (only `M` is a forwarded parameter) is conservatively rejected, since nothing computed within the body is provably a superset of "valid for the whole function." Secondary spans point at where each side is bound. Suggestion: pick arguments where the ordering actually holds, or drop the constraint if it isn't needed.
- `LIFETIME-006` `NonLocalBoundaryValue`: *Error*, same pass. "can't verify this value satisfies declared lifetime `L`". The conservative-reject case: the value resolves to a local, but that local is neither one of the caller's own parameters nor bound to any known loan (typically because it came from another call's return value — a call result is never itself registered as a loan). v1 can't prove it's fine, so it doesn't allow it, even in cases that are actually safe. Suggestion: bind it to a local first so its lifetime can be traced, or pass/store a fresh borrow directly.
