...
this delivery's scope; tracked in §12. (Since resolved in a later
delivery — `seed_param_type` threading; §12's entry reflects the
resolved state, this paragraph is left as the historical record of when
and how it was first found.)
...
| 7 | Real `outlives`/subset enforcement — connecting the already-landed loan/liveness borrow checker to the already-landed (but purely well-formedness-checked) named lifetime parameters, for the two boundary shapes that exist today: a function call and an `edge struct` construction. This is the piece this section and §6 both flag as still missing. | Scoped, not built — see `docs/OUTLIVES_RULES.md` for the full design (reuses `compute_reaches_before`/`compute_live_after` almost verbatim; new work is boundary constraint generation and `outlives` propagation, not a new borrow checker). Also closes this section's own `is_edge`/arena-escape-checker gap as a side effect once landed |
